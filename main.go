...
func main() {
	...
	timeout := flag.Duration("t", time.Second*30, "Maximum length of time dart-sass is allowed to compile")
	...
	opts = append(opts,
		sass.DartBinary(*binary),
		sass.Logger(l),
		sass.Timeout(*timeout),
	)

	r, err := sass.NewRunner(opts...)
	if err != nil {
		l.Error("error creating new runner", slog.Any("error", err.Error()))
		os.Exit(1)
	}

	l.Debug("created new runner")
